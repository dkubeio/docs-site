...
A coding agent's **package-app skill** does the packaging — it creates the Helm chart (wiring in any
platform resources your app uses, such as PostgreSQL or MinIO), builds and pushes the container images,
and publishes the chart to a Helm repository. This tutorial continues with the **document
data-extraction app** from the previous tutorial as the worked example.

## Prerequisites

- An app built and tested in your workspace — see
  [Building and hosting an app](./building-and-hosting-an-app-in-dkubex-workspace-with-a-coding-agent.md),
  which builds the document data-extraction app used here.
...
With the app built and working, package it. Start by prompting the agent:
...
In the workspace Terminal, sign in and refresh your token so it can push packages (this gives the
token read and write permission to packages):
