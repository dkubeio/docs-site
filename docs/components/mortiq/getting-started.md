...
This guide will help you set up and run MortIQ for the first time.
...
```bash
git clone https://github.com/dkubeio/mortiq.git
cd mortiq
```
...
DATABASE_URL=sqlite+aiosqlite:///./mortiq.db
...
docker run -d \
  ...
  -e DATABASE_URL=sqlite+aiosqlite:///./mortiq.db \
...
```bash
kubectl port-forward svc/mortiq 5300:5300
```
...
MortIQ includes three pre-configured demo applications:
